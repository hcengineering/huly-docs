...
However, you may wish to create your own space types to model different types of processes that you'll be tracking in your workspace. For example, could might space types for "Client projects", "Internal projects" and "Bug tracking", each of which would have its own roles and [task types](/advanced-settings/task-types).
...
Some properties and attributes for spaces are not currently customizable. Settings in the first block cannot be changed.

##### Creating roles

Default space types do not include roles, but you can create them by clicking the `+` button in the `ROLES` section. Enter a name for your role and click `Create`. Then, click on your role to select specific permissions. Currently, permissions available are "Update space", "Archive space" and "Forbid delete object".

Once your roles have been created, you'll be able to assign roles to team members for spaces using that space type. Further documentation on roles will be available soon.

##### Creating task types

Read an in-depth tutorial on updating and creating task types [here](/advanced-settings/task-types).
...
Once your custom space type has been created, you can add [task types](/advanced-settings/task-types) and roles.
