...
Let’s walk through an example using our **game design** system. **Types**, **Tags** and **Relations** are all covered in the following guides, but we'll take a peek at how they all appear on a `Character` Card:

!! INSERT IMAGE: Hero Card - shows all elements that are visible on the Card page

This is a Card created from the `Character` Type, which is derived from the `Game Component` Type. For more on how Types are derived, see [Creating a derived Type](/cards/types/create-a-type/#creating-derived-types).
