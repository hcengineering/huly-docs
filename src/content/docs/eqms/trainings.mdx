...
import trainingQuestions from '../../../assets/screenshots/eqms/training-questions.png';
import assignTraining from '../../../assets/screenshots/eqms/assign-training.png';

Trainings can be created from the Trainings module, or from the `Training` tab of any controlled document, where they can be attached directly to that document. Trainings consist of custom questions and answers that can be used to assess the knowledge of employees on a specific topic, and can be assigned to individuals or groups.
...
<Image 
src={trainingQuestions} 
alt='Creating training questions and setting a passing score' 
inferSize
quality='max'
/>
...
Navigate to the document you'd like to attach the training to and open the `Trainings` tab. Toggle the Training setting and select the training you'd like to attach. From here, you can select the roles and members to assign the training to, and set the due date and maximum number of attempts allowed.
...
Assignees will receive training requests in their email and Huly inbox. Assigned training requests can also be found in the Trainings module under the `My requests` sub-menu. Begin taking a training by clicking on it to open.
