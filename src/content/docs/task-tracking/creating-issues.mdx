...
- In keeping with best practices, an issue status should not need to be manually changed from "Todo" to "In Progress" or "Done". The status of an issue can be automatically be determined by the assigned team member's action item status. For more on how this process is automated (and why), read more about [action items](/task-tracking/creating-action-items) (previously called "todos") <a href="https://huly.io/blog/capturing-dynamic-team-workflows-with-action-items" target="_blank">here</a>.
