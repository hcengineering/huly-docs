...
Whenever an issue is created with a status of "Todo" and is assigned to a team member, the system automatically generates an [action item](../creating-action-items) in that member's Planner, allowing them to add it to their own schedule.
...
**Assignee**: Select a member to assign this issue to. When you create an issue assigned to a member, that member will recieve an [action item](../../task-tracking/creating-action-items) in their Planner that will allow them to add it to their own schedule.
...
- In keeping with best practices, an issue status should never need to be manually changed from "Todo" to "In Progress" or "Done". Rather, the status of an issue should automatically be determined by the assigned team member's action item status. For more on how this process is automated, read more about action items (previously called "Todos") <a href="https://huly.blog/huly-todos" target="_blank">here</a>.
